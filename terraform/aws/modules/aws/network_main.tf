resource "aws_vpc" "nomad_vpc" {
  ...
  tags = {
    Name           = "${var.clusterid}-vpc-${random_id.nomad_cluster.hex}"
    owner          = var.owner
...
resource "aws_internet_gateway" "nomad_igw" {
  ...
  tags = {
    Name           = "${var.clusterid}-igw-${random_id.nomad_cluster.hex}"
    owner          = var.owner
    created-by     = var.created-by
    nomad_join     = var.nomad_join
  }
...
resource "aws_subnet" "nomad_subnet" {
  ...
  tags = {
    Name           = "${var.clusterid}-subnet-${random_id.nomad_cluster.hex}"
    owner          = var.owner
    created-by     = var.created-by
    nomad_join     = var.nomad_join
  }
